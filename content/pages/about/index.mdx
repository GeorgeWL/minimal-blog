---
title: About
slug: '/about'
---

import About from '/components/about';
import ImageContainer from '/components/ImageContainer';
import { Image, Card, Link } from 'theme-ui';

<About />

If you want to contact me, email me at <a href='mailto:hello@georgewl.dev' target='_blank' rel='noreferrer noopener'>hello@georgewl.dev</a> or click on to my social media links and say hello.

### Github contributions

<Card>
  <Link href='//github.com/georgewl' target='_blank' rel='noopener noreferrer'>
    <Image src='//ghchart.rshah.org/GeorgeWL' title='My Github Contributions' />
  </Link>
</Card>

## Education

### BSc Web Systems Design

#### Sheffield Hallam

This is one of the few specialist courses in the UK focused on Web Technologies. Key Areas of Study:

- HTML5, CSS3
- JavaScript, PHP and ASP web scripting
- User Interface Design
- Programming
- Web Based Systems Architecture
- Web Security
- Creative And Critical Thinking
- Computer Architecture
- Design And Development Of Database Systems
- Computer Networking
- Project Management
- Web Frameworks
- Journal And Thesis Writing
- Information Management (Second year elective)
- Mobile Application Development (Final year elective)
- Cloud Databases And Cloud Computing (Final year elective)

### BTEC National Diploma Level 3

#### Mid-Cheshire College

This course is a strong foundation to build onto with University education or Apprenticeships. Course included the following modules:

- Communication and Employability Skills for IT
- Computer Systems
- Information Systems
- Project Planning with IT
- Database Design
- Spreadsheet Modelling
- Customisation of Web Pages
- Software Design and Development
- Human Computer Interaction
- Object Oriented Programming
- Computer Networks
- Website Production
- Event Driven Programming

### GCSEs A to C

#### Verdin High School

All relevant GCSEs to A-C levels.
